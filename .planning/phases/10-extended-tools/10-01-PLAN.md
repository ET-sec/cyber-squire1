...
Output: Finance commands log transactions and track progress toward financial targets payoff goal.
</objective>
...
<task type="auto">
  ...
  <verify>
    ...
    # "Log $50 for groceries" → amount: 50, category: Groceries
    # "Paid $500 debt" → amount: 500, is_debt_payment: true
  </verify>
...
<task type="auto">
  ...
  <action>
    ...
    **Debt Burn-Down Status**
    ...
    📉 Monthly burn rate: **${{burn_rate}}/month**
    🎯 Target: **$0** ({{target_debt}})
    📅 Projected payoff: **{{payoff_date}}**
...
<verification>
1. Send "Log $50 for groceries" → verify confirmation
2. Send "Paid $500 debt" → verify logged as debt payment
3. Send "Debt status" → verify balance and projection
4. Send "Spending summary" → verify category breakdown
