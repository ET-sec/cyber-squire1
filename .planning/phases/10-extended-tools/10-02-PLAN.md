...
<task type="auto">
  ...
  <action>
    ...
    VALUES
        ...
        ('YOUR_EC2_IP', 'EC2 instance', 'tigoue')
...
<task type="auto">
  ...
  <action>
    ...
    If is_allowed = 0:
    - Send error: "⛔ Target {{target}} is not whitelisted. Contact admin to add it."
    - Stop workflow
...
<task type="auto">
  ...
  <action>
    ...
    **Port Scan (Nmap):**
    ...
    {{/each}}
    {{else}}
    • No open ports detected in top 100
    {{/if}}
...
<task type="auto">
  ...
  <action>
    ...
    ```
    ⏱️ Rate limit: You scanned {{target}} {{minutes_ago}} minutes ago.
...
<verification>
1. Send "Scan unauthorized.com" → verify rejected
2. Send "Scan tigouetheory.com" → verify confirmation appears
3. Click Yes → verify scan executes
4. Verify Nmap results show open ports
5. Verify Nuclei results show vulnerabilities (if any)
6. Verify severity indicators (🔴 🟠) appear
7. Scan same target again immediately → verify rate limit
