#!/usr/bin/env python3
"""Security+ SY0-701 RED theme covers - matches CCNA gold design language."""
import os
...
def create_cover():
    ...
    # "SECURITY+" massive centered
    f_title = font('BigShoulders-Bold', 160)
    bbox = d.textbbox((0, 0), 'SECURITY+', font=f_title)
    tw = bbox[2] - bbox[0]
    title_x = (W - tw) // 2
    title_y = 80 * S
    d.text((title_x, title_y), 'SECURITY+', fill=WHITE, font=f_title)

    # "SY0-701" centered
    f_code = font('GeistMono-Bold', 64)
    bbox2 = d.textbbox((0, 0), 'SY0-701', font=f_code)
    tw2 = bbox2[2] - bbox2[0]
    code_y = title_y + (bbox[3] - bbox[1]) + 40 * S
    d.text(((W - tw2) // 2, code_y), 'SY0-701', fill=ACCENT, font=f_code)
    ...
    path = os.path.join(OUT_DIR, 'secplus-cover-red-1280x1280.png')
...
def create_thumbnail():
    ...
    # "SEC+" large (shorter than SECURITY+ to fit thumbnail)
    f_title = tfont('BigShoulders-Bold', 120)
    bbox = d.textbbox((0,0), 'SEC+', font=f_title)
    tw = bbox[2] - bbox[0]
    th = bbox[3] - bbox[1]
    d.text(((W-tw)//2, H//2 - th - 30*T), 'SEC+', fill=WHITE, font=f_title)

    f_code = tfont('GeistMono-Bold', 44)
    bbox = d.textbbox((0,0), 'SY0-701', font=f_code)
    tw = bbox[2] - bbox[0]
    d.text(((W-tw)//2, H//2 + 10*T), 'SY0-701', fill=ACCENT, font=f_code)
    ...
    path = os.path.join(OUT_DIR, 'secplus-thumb-red-600x600.png')
    final.save(path, 'PNG', optimize=True)
...
if __name__ == '__main__':
    create_cover()
    create_thumbnail()
    for f in ['secplus-cover-red-1280x1280.png', 'secplus-thumb-red-600x600.png']:
