...
| Resume PDF | the resume build reads `metrics.yaml` by hand; GTA Phase 11 flags drift | flagship in `~/Library/Mobile Documents/com~apple~CloudDocs/` |
...
<!-- METRIC:stack.containers_live -->3<!-- /METRIC -->
...
Generated by `scripts/build_metrics.py`. Canonical values as read on 2026-09-15:

```yaml
stack:
  containers_live: 8               # owner-approved, re-verified from docker ps
  containers_designed: 19          # the service list in the master compose file
  workflows_n8n_exported: 14       # derived: the exports in the engine directory
  workflows_github_actions: 18     # derived: ls .github/workflows/
...
- **It does not edit the resume PDF.** The resume build is a manual step and stays one. GTA Phase 11 flags resume drift and leaves the fix to the operator.
...
- **It does not check Notion, LinkedIn, Skool, or Gumroad.** External SaaS surfaces are out of scope, and the numbers shown on them are reconciled by hand against `metrics.yaml`.
...
| Portfolio HTML marker injection | done, `sync_portfolio.py` writes between the METRIC markers |
| Resume source build reads metrics | out of scope, the resume build is a manual step |
| Diagram gen_*.py coverage | 4 of 18 PNGs have generators, the rest are hand-authored |
| IR playbook reference auto-check | out of scope here, the GTA Phase 2 audit job covers it |
