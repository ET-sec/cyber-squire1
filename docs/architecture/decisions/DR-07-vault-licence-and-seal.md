...
| B. Switch to OpenBao | The MPL fork of Vault 1.14. It mirrors the seal types this design needs, including the cloud KMS seal used here | Recorded, not taken: the fork tracks a 2023 base and the platform gains nothing today from moving |
...
2. **The seal is the cloud KMS through the instance principal, with its own key.** The storage key that encrypts state and backups is never reused for a second purpose. The seal key is a separate AES key in the same vault, and the instance's dynamic group is granted `use keys` scoped to that key's id and nothing wider. No plaintext unseal material exists on the host: the barrier and recovery keys sit on the volume wrapped by the cloud key, and only the instance identity can unwrap them. The recovery shares and the initial root token went to the secrets manager of record on initialization and to no file.
...
5. **Two engines, nothing else.** The database secrets engine issues short-lived Postgres credentials through one role, `cd-app-readwrite`, with a one hour lease and a two hour maximum. The transit engine holds one key for wrapping the orchestration engine's encryption key. No other engine and no auth method beyond the token backend is enabled.
...
The instance identity is ambient: any container that can reach the metadata service on the host can call the KMS with it, so the seal key and the backup writer share one identity; a host firewall rule scoping the metadata address to the secrets manager is the residual that closes it. The dynamic group is pinned to one instance id, so a rebuilt instance cannot unseal or back up before the identity Terraform is re-applied. If the KMS key is deleted or the policy is revoked, Vault cannot unseal on its next restart. The recovery shares do not unseal an auto-sealed Vault; they authorize operations such as generating a new root token. Recovery is a seal migration, which needs the old seal reachable, so the key's deletion is guarded by the key vault's own deletion window and by the same 30-day retention the backups carry. If the database role's Postgres user loses its rights, dynamic credentials stop issuing and every consumer falls back to its deploy-time credential, which still exists today. The edge now trusts an email claim from either provider, so an administrator of the identity provider, or a write to its tables, can mint an identity the edge accepts; the identity provider's administration console is unrouted, its database role is scoped to its own schema, and the six agent clients are disabled until a consumer ships. Splitting the allow policy per provider with a realm group requirement is the follow-up.
...
| 3 | Terraform plan after the seal key and its policy were applied | no changes | "No changes. Your infrastructure matches the configuration." |
| 4 | One read of `database/creds/cd-app-readwrite` | a lease of one hour, a username and a password present, the role visible in Postgres | lease 3600, both present, the credential itself never printed; one dynamic role visible in the database afterwards |
...
| 7 | A browser login through the edge: the provider link on the edge login page, the sign-in form, the code exchange, the dashboard | the orchestration dashboard loads | landed on the dashboard on the second attempt; the first failed while the identity hostname sat in the geo-fence, which is why that hostname stays out of it |
| 8 | The volume on the host | no operator-held share, no plaintext token | the volume holds only the server's own barrier files, wrapped by the cloud key, and the audit log; no share and no token an operator could use |
...
- A host firewall rule scoping the metadata address to the secrets manager's container, so the instance identity is not reachable from every container on the bridge networks.
- The edge allow policy split per provider, with the identity provider's path requiring a realm group rather than an email claim alone.
- The identity provider's hostname stays outside the WAF geo-fence: with it inside, the edge's own code exchange failed ("could not fetch user information from the identity provider"), because the edge calls the token and userinfo endpoints from its own network. A rate limit on the login form's path family is the control to add instead.
- A second audit device is enabled (the process stdout, collected by the monitoring agent) so a single device that cannot write does not block every request.
