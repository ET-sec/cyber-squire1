...
## Options considered
1. **Customer-managed key (CMK), software-protected, in OCI Vault.** Unlimited software key versions, verified against Oracle's published Vault limits, so rotation is never rationed; Terraform-managed, revocable, auditable. Added risk: self-inflicted key deletion, mitigated by OCI's mandatory deletion waiting period and by the key existing only as Terraform config (no hand-deletes).
...
1. **The retention rule is UNLOCKED.** A locked rule is irreversible for its whole duration; in a single personal tenancy with no second account to fall back on, a sizing mistake with a locked rule cannot be undone. Unlocked means a tenancy admin could remove the rule and then delete objects, so the guarantee today is "ransomware with stolen writer credentials fails" and "admin fat-fingers fail," not "malicious admin fails." Re-evaluate locking at phase close.
...
- Retention mis-sized too long: storage grows past the tenancy's object storage allowance; at ~28KB/day this is arithmetic, not risk.
