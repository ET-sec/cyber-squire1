...
`nodes/<slug>.yaml` makes the boxes on a view clickable. One entry per box, keyed by the exact label text on the drawing: what the box is (one sentence), the NIST 800-53 controls it carries, a `status_note` that describes the element in the reference design, and evidence links as `{label, path, line}` into this repo. A node entry carries no status vocabulary at all, and `sync_views.py` refuses a table that adds one. The only status any drawing renders is the control chip generated from its row in the System Security Plan, which carries that row's own wording and can read Implemented or Inherited and nothing else. `sync_views.py` wraps each matching rect and label in a focusable group, attaches the SSP row (name, status, line number) to every control at sync time, and emits the JSON the panel reads; the panel itself is one CSS block and one script that the sync writes into both the standalone page and `index.html` between the `NODES:css` and `NODES:js` markers.

All seven views have tables: `topology` (31 boxes), `multi-cloud` (25), `threat-model` (6 attacker positions plus 31 walls; the STRIDE letters and the residual chips stay unwrapped), `identity-access` (25 entries for 28 boxes; `operator` and `Squire` are drawn more than once and carry `repeat: true`, so every occurrence opens the same panel), `ai-trust` (21 boxes plus the 7 OWASP tag chips, written by hand against the catalog rows), `control-layers` (42 barrier boxes plus a generated entry per control chip), and `authorization-boundary` (44 boxes). Entries inherit across tables with `from: <slug>/<id>` (a threat-model wall inherits the control-layers barrier it draws, an identity-view principal inherits the topology service), so a correction lands in one place.
