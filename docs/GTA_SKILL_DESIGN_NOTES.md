...
- NIST 800-53 Rev 5 control IDs plus names plus descriptions
...
- HIPAA Security Rule (45 CFR sections 164.308/310/312/316)
...
Why this matters: a big-scope GTA could take 30 to 60 minutes. Context limits or network drops should not waste the run.

### 12. Three review personas

External Reader plus Technical Reviewer plus Stranger.

- **External Reader pass**: scores credibility for a non-technical audience. Surfaces sentences that would trip up a reader unfamiliar with the project.
- **Technical Reviewer pass**: scores technical defensibility. Surfaces sentences a technical expert would push back on.
- **Stranger test**: reads one doc cold with no project context. If they cannot figure out what the project is and why it matters from the doc alone, the doc fails. Catches insider-jargon docs that only make sense internally.

Why this matters: three personas cover the three failure modes (looks bad to a generalist, looks shallow to an expert, looks like internal noise to anyone else).

### 15. Treat single-agent output as a draft

The skill bakes in the assumption that any single agent confidently hallucinates sometimes. Default mode:

- Single-agent finding = draft
- Two-agent agreement = candidate fix
- Three sources (two agents plus ground truth file) = applied fix

Why this matters: structural defense against the most common LLM failure mode.

### 16. Hold-position protocol in debates

Per the existing voice rules, an agent must not switch positions because another agent pushed back. Only new evidence shifts the position. The debate protocol enforces this: each "switch" must cite specific new evidence not present in the original claim.
...
Every completed GTA produces a small signed file: "scope X, audited {date}, GTA v{N}, found {Y} corrections." Sigstore or cosign. Compliance evidence and a verifiable audit trail.

### 11. Drift diff between runs

After each run, compute the diff against the previous run's state. "Since the last audit, X new claims drifted, Y were closed, Z new TODOs appeared." Surfaces drift velocity.

### 13. Pull-request mode

When run on a branch, the skill produces PR-shaped output: each correction as a separate commit with a real message, organized for review. Makes the discipline mergeable.

### 14. Notification summary

Post a brief summary to a channel at the end: scope, fixes applied, time taken, TODOs left, debates pending.
...
For high-impact docs (SSP, threat models, Executive Summaries), run a second pass with a different model. Opus audits, then Sonnet audits independently. Disagreements between models surface the hardest cases.

### 6. External fact-checking with WebFetch

When a doc cites NIST 800-53 AC-2 or OWASP LLM01, the agent WebFetches the actual standard and verifies the citation. Adds latency. Worth it for compliance-grade output.

### CI gate

Wire the GTA to GitHub Actions on every PR that touches docs/. Block merge until a current GTA sidecar exists with zero outstanding WRONG entries.

---

## Known failure modes the skill defends against

This section is explicit because the skill must defend against LLM limitations, not assume the agents are perfect.

**Confident hallucination.** Items 4, 5, 15 are the antidotes. Confidence scores force calibration. Multi-model audits catch shared blind spots. Treating single-agent outputs as drafts means no single-agent claim becomes a fix without a second pair of eyes.

**Position switching under social pressure.** Item 16 is the antidote. The debate protocol holds position unless new evidence appears.

**Conflating "I applied the fix" with "the fix is in the file".** Items 2, 3 are the antidotes. Verification phase reads the actual diff, not the agent's claim.

**Missing things when context is long.** Items 9, 17 are antidotes. Checkpoint and resume keeps each phase short. Batched ground-truth snapshots avoid re-reading the same file 6 times.

**Voice drift even with rules.** Item 1 antidote: deterministic Python sweep for em dashes and AI patterns catches more than a single LLM pass would.

**Precision loss in big clusters.** Smaller clusters with more agents reduce per-agent context load. When in doubt, split the cluster.

---

## Open design questions

1. **Where does the REPO_MAP.yaml live?** Probably at repo root: `REPO_MAP.yaml`. Gitignored or committed? Argument for committed: provenance. Argument for gitignored: it leaks scope and structure. Default to gitignored, with a `REPO_MAP.example.yaml` committed for reference.
...
4. **How do debates surface to the operator?** Inline in the master synthesis doc, with the full reasoning narrated by the main thread. NOT as a separate file. Visibility means they get seen, otherwise they did not happen.
...
6. **Cost ceiling per run?** Worth a hard cap. If a single GTA run exceeds expected resource use, pause and ask. Prevents runaway runs.
...
1. REPO_MAP.yaml schema plus scanner script (Phase 0 spine)
2. Skill scaffold in `~/.claude/skills/gta/`
3. Ground truth catalog files (JSON for NIST, OWASP, MITRE)
4. Deterministic Python sweep script (em dashes, AI patterns, double-hyphens)
...
9. Personas (external reader, technical reviewer, stranger)
