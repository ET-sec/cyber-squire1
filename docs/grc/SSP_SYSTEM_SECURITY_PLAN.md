...
| CM-3 | Configuration Change Control | Implemented | All configuration changes follow a controlled process: (1) changes are submitted as pull requests; (2) `terraform-pr.yml` runs format, validate, Checkov, and the OPA policy self-test against fixture plans on every pull request, with no cloud credentials, and posts the results as a PR comment; (3) merge to main runs the security pipeline and `terraform-main.yml`, which plans against remote state with a session minted by the OIDC token exchange and runs the OPA policies against that real plan; (4) apply from CI sits behind a reviewed environment and is switched off until the CI identity is granted write, so apply runs from the operator workstation today; (5) a lifecycle guard and an OPA deny protect stateful resources from deletion. | `.github/workflows/terraform-pr.yml`; `.github/workflows/terraform-main.yml`; `.github/workflows/security.yml` |
...
| CM-5 | Access Restrictions for Change | Implemented | Branch protection requires a pull request and the security scan before merge to main, administrators included; the repository has one maintainer, so no second approver is required (recorded under AC-5). CI secrets are encrypted repository secrets not exposed to forks. The only cloud identity CI can assume is a session minted on main by the OIDC token exchange, and that identity is read-only; apply runs from the operator workstation with short-lived credentials, and the CI apply job stays switched off behind a reviewed environment until the identity is granted write. SSH to the host is key-only from the allowlisted range. | Repository branch protection settings; `.github/workflows/terraform-main.yml` (production environment, `CI_APPLY_ENABLED`); `.github/workflows/drift-check.yml` |
