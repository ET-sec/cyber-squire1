...
| `Vector` | Audit log shipper - receives access gateway audit events via mTLS, forwards to monitoring | internal | net-monitoring |
...
| AU-5 | Response to Audit Logging Process Failures | Implemented | Container health checks detect logging subsystem failures. If `svc-detection` or `svc-monitor` containers fail, the container down monitor triggers a critical alert (threshold: fewer than 4 running containers). `svc-detection-router` health check validates event routing is operational. `Vector` health check confirms log pipeline agent is accepting events. All alerts route to the SOAR platform via webhook relay. | `terraform/*/monitoring.tf` (container_down); `docker-compose.yaml` (healthcheck definitions) |
