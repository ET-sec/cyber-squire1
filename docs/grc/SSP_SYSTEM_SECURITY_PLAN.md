...
- The DigitalOcean's physical infrastructure, hypervisor, and network backbone
...
The platform operates as a 13-service containerized stack deployed on a single 4-vCPU / 8 GB VPS running Ubuntu 24.04 LTS. All services communicate over an isolated Docker bridge network (`internal-net`). The only public ingress path is a zero-trust tunnel (`svc-tunnel`) operating in host networking mode. No service ports are directly exposed to the public internet.
...
```
       ...
       example-ops.com   example-ops.com
        ...
        |                     |
   +----------+----------+     +--------+---------+
   | svc-automation:5678 |     | svc-gateway:3080 |
   ...
   +---------------------+   +-------------+-----------+
                        |
   +---------------------+   +-------------+-----------+
   | svc-secrets:8200  |   | Fluentd     |
   | (secrets engine)  |   | (log pipeline -> monitoring) |
   +---------------------+   +-------------------------+

   +---------------------+   +-------------------------+
   | svc-identity:8080  |   | svc-detection      |
   | (identity provider v26) | | (Falco) |
   +---------------------+   +-------------+-----------+
                        |
   +---------------------+   +-------------+-----------+
   | svc-llm:11434    |   | svc-detection-router  |
   | (LLM engine)    |   | (-> Datadog) |
   +---------------------+   +-------------------------+

   +---------------------+   +-------------------------+
   | svc-transcription  |   | svc-monitor       |
   | :8000 (transcription engine) | | (agent, pid:host) |
   +---------------------+   +-------------------------+
...
|---------|------|------|---------|
| `svc-db` | PostgreSQL 16 — persistent workflow state and application data | 5432 | internal-net |
| `svc-automation` | Workflow orchestration engine (SOAR) — 16+ automation actions | 5678 | internal-net |
| `svc-llm` | Local LLM inference — private AI processing | 11434 | internal-net |
| `svc-transcription` | Voice-to-text — local transcription, no external API calls | 8000 | internal-net |
| `svc-secrets` | Secrets engine — centralized secrets, AppRole auth, short-lived leases | 8200 | internal-net |
| `svc-identity` | Identity provider (v26) — SSO, RBAC (3 roles), OIDC provider | 8080 | internal-net |
| `svc-gateway` | Access gateway (v18) — zero-trust SSH, session recording, TOTP MFA, JIT access | 3080/3023/3025 | internal-net |
| `svc-monitor` | Observability agent — container logs, metrics, process monitoring | N/A | internal-net |
| `svc-detection` | Runtime detection engine — eBPF-based syscall monitoring across all containers | N/A | internal-net |
| `svc-detection-router` | Routes runtime detection events to Datadog (Events + Logs API) | 2801 | internal-net |
| `Fluentd` | Log pipeline agent — receives access gateway audit events via mTLS, forwards to monitoring | 8888 | internal-net |
| `svc-event-shipper` | Streams access gateway audit events via gRPC to `Fluentd` using mTLS | N/A | internal-net |
| `svc-tunnel` | Zero-trust tunnel — sole public ingress path, read-only rootfs | host | host |
| `svc-ai-gateway` | AI agent gateway — standalone container, not Compose-managed | 18789-18790 | internal-net |
...
4. `svc-tunnel` routes `automation.example-ops.com` to `svc-automation:5678` and `ssh.example-ops.com` to `svc-gateway` or local SSH.
...
7. `svc-detection-router` forwards runtime detection events to the Datadog's Events and Logs APIs.
...
All 13 Compose-managed services operate on the `internal-net` Docker bridge network. This network is isolated from the host network stack. Only `svc-tunnel` uses host networking mode, as required for tunnel operation. No container ports are bound to the host's public interface except through the tunnel ingress path.

The DigitalOcean's firewall restricts inbound traffic to ICMP (health checks) and TCP/22 (emergency SSH). All application access is brokered through the zero-trust tunnel, which terminates TLS at the tunnel provider's edge and re-originates connections to local services.
...
| **Network** | Bridge driver (`internal-net`), host mode for `svc-tunnel` only |
| **Image Sources** | Docker Hub, Quay.io, AWS ECR Public |
...
| **Inherited** | Control is satisfied by the DigitalOcean or external service |
...
| CM-2 | Baseline Configuration | Implemented | The system baseline is defined by: (1) Terraform HCL files defining all cloud resources (16 .tf files across compute, networking, firewall, DNS, tunnel, monitoring, dashboard, secrets pipeline, SSH keys, project, outputs, checks, variables, providers, templates); (2) `docker-compose.yaml` defining all 13 services with pinned image versions where applicable; (3) Policy engine (Rego) rules defining 8 configuration constraints. | `terraform/*/` (16 .tf files); `docker-compose.yaml`; `terraform/*/policy/` (8 .rego files) |
...
| CM-8 | System Component Inventory | Implemented | The system component inventory is maintained through: (1) `docker-compose.yaml` as the authoritative service manifest (13 services with pinned images); (2) Terraform state as the authoritative cloud resource inventory; (3) SBOM generation (SPDX-JSON) for the repository and 6 container images on every merge to main, with 90-day artifact retention. | `docker-compose.yaml`; Terraform state (remote backend); `.github/workflows/security.yml` (sbom job) |
...
| MP-4 | Media Storage | Inherited | Physical media storage is managed by the DigitalOcean under their shared responsibility model. The platform uses the provider's block storage (SSD) and object storage (with versioning enabled). | Cloud provider security documentation |
...
| Control ID | Control Name | Status | Implementation Description | Evidence Location |
|-----------|-------------|--------|---------------------------|-------------------|
| PE-1 | Policy and Procedures | Inherited | Physical and environmental protection is entirely the responsibility of the DigitalOcean. The Organization does not operate physical data center facilities. | Cloud provider SOC 2 Type II report; DigitalOcean security documentation |

*Note: All PE family controls (PE-2 through PE-20) are inherited from the DigitalOcean under the shared responsibility model. The provider maintains physical access controls, environmental controls, and facility security for the data center infrastructure.*
...
| SC-7 | Boundary Protection | Implemented | Network boundary protection is implemented through: (1) Cloud firewall as the outermost perimeter; (2) Zero-trust tunnel as the sole public ingress path — no application ports exposed directly; (3) Docker bridge network (`internal-net`) isolating all services from the host network; (4) `svc-tunnel` is the only container with host networking; (5) Infrastructure policy denies firewall rules allowing public inbound on non-HTTPS ports (SSH exception documented as accepted risk). | `terraform/*/firewall.tf`; `terraform/*/tunnel.tf`; `docker-compose.yaml` (networks); `terraform/*/policy/deny_public_firewall.rego` |
...
| SC-20 | Secure Name/Address Resolution Service | Inherited | DNS resolution is provided by the DigitalOcean and tunnel provider. DNSSEC is managed at the DNS provider level. | Cloud provider DNS; Tunnel provider DNS |
...
*The following SC controls were identified as missing during QC coverage review and are documented here for completeness:*
...
| SC-21 | Secure Name/Address Resolution Service (Recursive or Caching) | Inherited | DNS recursive resolution is provided by the DigitalOcean's default DNS resolver on the VPS. The platform does not operate its own recursive DNS infrastructure. | Cloud provider network configuration |
| SC-22 | Architecture and Provisioning for Name/Address Resolution Service | Inherited | DNS architecture and provisioning are managed by the Cloudflare (authoritative DNS) and the DigitalOcean (recursive resolution). The platform delegates all DNS operations to these providers. | Cloudflare DNS configuration; DigitalOcean network settings |
