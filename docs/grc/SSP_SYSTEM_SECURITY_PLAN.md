...
| `Vector` | Audit log shipper: receives access gateway audit events via mTLS, forwards to monitoring | internal | net-core, net-monitoring |
| `svc-event-shipper` | Streams access gateway audit events over HTTPS to `Vector` using mTLS | N/A | net-core, net-monitoring |
...
4. `svc-event-shipper` streams access gateway audit events over HTTPS to `Vector` using mutual TLS (mTLS).
5. `Vector` forwards audit events to the external Datadog's Logs API.
...
2. `svc-monitor`, `svc-detection-router`, and `Vector` transmit telemetry to `datadoghq.com`.
...
| **Inherited** | Control is satisfied by the cloud provider or an external service |
...
| AC-4 | Information Flow Enforcement | Implemented | Information flow is enforced through Docker network segmentation. All services communicate over three segmented Docker bridge networks (`net-core`, `net-ai`, `net-monitoring`). `svc-tunnel` is the sole ingress path (host networking). No container ports are bound to the host's public interface. Egress from `net-core` is unrestricted for operational requirements (API integrations, telemetry). `net-ai` is declared `internal: true`, so its members have no route off the host and the local model is reachable only from a container that also sits on `net-ai`. | `docker-compose.yaml` (network definitions, `net-ai` internal); `terraform/cd-oci-infrastructure/networking.tf` (egress rule) |
...
| AC-17 | Remote Access | Implemented | Remote administration reaches the host two ways only: (1) the zero-trust tunnel, where the edge enforces a one-time PIN identity check before a request reaches an internal console; (2) break-glass SSH on port 22, key-only, accepted solely from the operator's allowlisted source addresses, set as the Terraform variable `ssh_allowed_cidrs` and applied to the host security list. No other inbound port exists on the host. The previous provider's open SSH rule was retired with the 2026-08 migration (POAM-021, closed). | `terraform/cd-oci-infrastructure/networking.tf` (ingress from `ssh_allowed_cidrs`); `terraform/cd-oci-infrastructure/variables.tf`; `terraform/cd-cloudflare-edge/access_n8n.tf` |
...
| AU-5 | Response to Audit Logging Process Failures | Implemented | Container health checks detect logging subsystem failures. If `svc-detection` or `svc-monitor` containers fail, the container down monitor triggers a critical alert (threshold: fewer than 4 running containers). `svc-detection-router` health check validates event routing is operational. `Vector` health check validates the pipeline configuration it runs, and the counters on its exporter are the signal that events are received and sent. All alerts route to the SOAR platform via webhook relay. | `terraform/*/monitoring.tf` (container_down); `docker-compose.yaml` (healthcheck definitions) |
...
| AU-9 | Protection of Audit Information | Implemented | Audit logs shipped to the external Datadog are protected by the platform's access controls. Access gateway audit events are transmitted to `Vector` via mTLS (mutual TLS with client certificate authentication), preventing tampering in transit. `svc-detection-router` operates with a read-only rootfs. Local container logs are accessible only to root. Audit logs exported to object storage use versioning to prevent overwrites. | `docker-compose.yaml` (svc-detection-router: read_only: true); mTLS certificates in `Vector` and `svc-event-shipper` volume mounts |
...
| CA-8 | Penetration Testing | Partially Implemented | Testing is self-directed, not independent: (1) DAST against the automation console with OWASP ZAP baseline scans, unauthenticated and authenticated, on a schedule in CI; (2) a documented penetration test self-assessment; (3) AI red-team cycles against the SOC agent with findings recorded and fed to the POA&M. No third-party assessor has tested the platform; that is the gap this row records. | `docs/grc/PENTEST_SELF_ASSESSMENT.md`; `docs/grc/DAST_METHODOLOGY.md`; `docs/grc/AI_RED_TEAM_PLAN.md`; `docs/grc/REDTEAM_RESULTS.md`; `.github/workflows/dast-zap.yml` |
...
| CM-3 | Configuration Change Control | Implemented | All configuration changes follow a controlled process: (1) changes are submitted as pull requests; (2) `terraform-pr.yml` runs format, validate, Checkov, and the OPA policy self-test against fixture plans on every pull request, with no cloud credentials, and posts the results as a PR comment; (3) merge to main runs the security pipeline and `terraform-main.yml`, which plans against remote state with a session minted by the OIDC token exchange and runs the OPA policies against that real plan; (4) apply from CI sits behind a reviewed production environment, and apply runs from the operator workstation; (5) a lifecycle guard and an OPA deny protect stateful resources from deletion. | `.github/workflows/terraform-pr.yml`; `.github/workflows/terraform-main.yml`; `.github/workflows/security.yml` |
| CM-4 | Impact Analyses | Implemented | Impact analysis runs twice. On the pull request, Checkov evaluates the HCL and the OPA policies are proven against a clean and a violating fixture plan. After merge, `terraform-main.yml` produces the real plan against remote state and evaluates it with the same policies (public firewall rules, missing encryption, deletion of stateful resources, root SSH keys); the change set is recorded as resource addresses and actions, and plan detail never reaches the public log. | `.github/workflows/terraform-pr.yml` (Checkov, OPA self-test); `.github/workflows/terraform-main.yml` (real plan, OPA); `terraform/cd-oci-infrastructure/policy/*.rego` |
| CM-5 | Access Restrictions for Change | Implemented | Branch protection requires a pull request and the security scan before merge to main, administrators included; the repository has one maintainer, so no second approver is required (recorded under AC-5). CI secrets are encrypted repository secrets not exposed to forks. The only cloud identity CI can assume is a session minted on main by the OIDC token exchange, and that identity is read-only; apply runs from the operator workstation with short-lived credentials, and the CI apply job sits behind a reviewed production environment. SSH to the host is key-only from the allowlisted range. | Repository branch protection settings; `.github/workflows/terraform-main.yml` (production environment, `CI_APPLY_ENABLED`); `.github/workflows/drift-check.yml` |
...
| CM-14 | Signed Components | Implemented | Every container image in both compose files is pinned by digest and checked in CI against a per-publisher signature policy on pull requests and on main, with no soft-fail: publishers that sign (Falco and Falcosidekick keyless, signer identity pinned to their GitHub repositories; Teleport against its RSA key) must verify; publishers that do not sign (12 of 16 upstream repositories as of 2026-09-06) are recorded as unsigned and covered by the digest pin plus vulnerability scanning of the pinned bytes; an image with no policy entry fails the job. Renovate can only automerge a bump that passes this job. | `scripts/verify_image_signatures.py`; `.github/image-signers.json`; `.github/workflows/security.yml` (Container Image Verification job); `renovate.json` |
...
| CP-4 | Contingency Plan Testing | Implemented | Two tests run on a calendar: (1) a monthly timed restore of the latest database dump into a scratch container on the host, with the time to a usable database logged (first run 2026-08-31: 76 tables, 5 seconds); (2) the tabletop exercise that walks the incident phases from detection through recovery. Individual service recovery is exercised through operational restarts. | `docs/architecture/decisions/DR-03-data-protection.md` (verification record); `docs/grc/POLICY_DISASTER_RECOVERY.md` (restore test); `docs/grc/TABLETOP_EXERCISE.md` |
| CP-6 | Alternate Storage Site | Implemented | Terraform state is stored in versioned object storage (remote backend with versioning enforced by infrastructure policy). Database backups are written to a dedicated backup volume (`CD_BACKUPS`) and uploaded nightly by instance principal to a key-encrypted object storage bucket off the host, under a 30 day retention rule with a monthly timed restore. | `terraform/*/terraform.tf` (backend config); `docker-compose.yaml` (CD_BACKUPS volume); `terraform/*/policy/deny_no_encryption.rego` (versioning enforcement) |
...
| CP-9 | System Backup | Implemented | The database is dumped nightly from the host into an off-host object storage bucket encrypted under a customer-managed key. The bucket carries a 30-day retention rule, so a dump cannot be modified or deleted for 30 days even by the writer, and the writer authenticates by instance principal under a grant that can create and read objects but not delete them; the backup job holds no stored cloud credential. Terraform state is versioned in a separate remote bucket under the same key. Docker volumes for persistent data stay on local disk; the nightly dump is the off-host copy. Infrastructure policy warns when a plan drops the retention rule. | `terraform/cd-oci-infrastructure/data_protection.tf` (backup bucket, retention rule, writer identity and grant); `docs/architecture/decisions/DR-03-data-protection.md`; `terraform/cd-oci-infrastructure/policy/warn_backup.rego` |
| CP-10 | System Recovery and Reconstitution | Implemented | Reconstitution runs from source control plus the off-host dump: (1) `terraform apply` against remote state for cloud resources, with a lifecycle guard on the instance and an OPA policy that fails any plan deleting a stateful or key custody resource; (2) `docker compose up -d` for services; (3) database restore from the latest dump, the same procedure the monthly timed restore exercises; (4) re-import of the `svc-identity` realm. The 2026-08 provider migration was a full reconstitution on this path. | `terraform/cd-oci-infrastructure/compute.tf` (lifecycle); `terraform/cd-oci-infrastructure/policy/deny_missing_prevent_destroy.rego`; `terraform/cd-oci-infrastructure/data_protection.tf`; `docs/architecture/decisions/DR-03-data-protection.md`; `docker-compose.yaml`; identity-import volume |
...
| MP-5 | Media Transport | Implemented | All data in transit is encrypted: (1) Zero-trust tunnel provides TLS encryption for ingress; (2) mTLS between `svc-event-shipper` and `Vector`; (3) HTTPS for all outbound API communications to Datadog, secrets manager, and AI providers; (4) SSH uses Ed25519 keys for session encryption. | `docker-compose.yaml` (mTLS cert mounts); `terraform/*/tunnel.tf` |
...
| SA-15 | Development Process, Standards, and Tools | Implemented | One development process covers infrastructure, compose, and application code: every change is a pull request; a pre-commit hook runs secret scanning and fails closed; the merge pipeline runs secret, vulnerability, static-analysis, and signature checks; every workflow action is pinned to a commit SHA; the SOC agent's toolchain is `uv`, `ruff`, `mypy`, and `pytest`, declared in its project file, with no proprietary build server. | `docs/grc/SECURE_SDLC.md`; `.githooks/pre-commit`; `.github/workflows/security.yml`; `builds/squire/pyproject.toml` |
...
| SC-12 | Cryptographic Key Establishment and Management | Implemented | Cryptographic keys are managed through: (1) SSH keys managed via Terraform (`cloud_ssh_key` resource); (2) `svc-automation` encryption key stored in secrets manager; (3) `svc-gateway` identity certificates with defined TTL (1-year for event-handler); (4) mTLS certificates for event pipeline; (5) secrets manager seal: unsealed by the cloud KMS through the instance identity with a dedicated key, the recovery shares held in the secrets manager of record, no unseal material on the host| `terraform/*/ssh.tf`; Secrets manager configuration; mTLS certificate chain |
...
### 5.17 SR - Supply Chain Risk Management

| Control ID | Control Name | Status | Implementation Description | Evidence Location |
|-----------|-------------|--------|---------------------------|-------------------|
| SR-3 | Supply Chain Controls and Processes | Implemented | Upstream images are classified into three tiers with a written update policy per tier: state-bearing images update by reviewed pull request only; stateless images and patch bumps may automerge once signature verification and the scan pipeline pass; major versions stay manual. Every image is pulled from a named registry namespace by digest, and the compose file header records the tier policy. | `renovate.json`; `COREDIRECTIVE_ENGINE/docker-compose.yaml` (tier policy header); `.github/image-signers.json` |
| SR-4 | Provenance | Implemented | Provenance is recorded three ways on every merge: a software bill of materials for the repository and for each pinned image digest, the digest pin itself in the compose files, and the signature policy that names the expected signer identity per publisher. The bills of materials are retained as CI artifacts. | `.github/workflows/security.yml` (Generate SBOMs job); `COREDIRECTIVE_ENGINE/docker-compose.oci-core.yaml` (digest pins); `.github/image-signers.json` |
| SR-11 | Component Authenticity | Implemented | Image authenticity is checked in CI against the publisher's own signing identity or key where the publisher signs, and a component that cannot be verified against its policy entry blocks the merge. For publishers that do not sign, authenticity rests on the digest pin: the bytes that were scanned are the bytes that run. | `scripts/verify_image_signatures.py`; `.github/keys/teleport-oci-key-2023-05.pub`; `.github/workflows/security.yml` (Container Image Verification job) |
...
| POA-006 | Off-site backup replication not implemented | CP-6, CP-9 | Medium | 2026-08-31 | Closed - nightly dump to a retention-locked, key-encrypted off-host bucket (`terraform/cd-oci-infrastructure/data_protection.tf`, DR-03) |
...
| POA-009 | Emergency SSH (port 22) open to 0.0.0.0/0 | SC-7 | Medium | 2026-09-06 | Closed - the open rule was retired with the provider migration; SSH is accepted from the allowlisted range only (`terraform/cd-oci-infrastructure/networking.tf`, POAM-021) |
...
|--------|--------------|-------|
| This SSP (parent) | 140 | Platform-wide NIST 800-53 Moderate baseline (unique control ids across both SSPs after the 2026-09-06 re-baseline) |
| SQUIRE_SSP.md (child) | 36 | Squire-scoped controls with inheritance annotations |
| Combined coverage | 176 | Platform + Squire subsystem |
...
| `SQUIRE_MODEL_CARD.md` | Mitchell et al. model card for Fable 5, Opus 5, Voyage AI voyage-3-large |
...
|------------|-----------|---------|-------------------|
| Anthropic API | Outbound HTTPS from svc-squire | Opus 5 classify, Fable 5 draft + critique | SA-9, SC-7, SC-8 |
...
| 1.3 | 2026-09-06 | System Owner | Re-baseline against the current provider: AC-4 egress stated per network; AC-17, CA-8, CM-14, SA-15 rows and a new SR family (SR-3, SR-4, SR-11) added; CP-4, CP-9, CP-10 to Implemented on the retention-locked off-host backup and the monthly timed restore; POA-006 and POA-009 closed; legend no longer names the previous provider. |
