...
- [ ] **Container restart loop**: any container restarting >3 times in 10 minutes (Datadog metric monitor; no Sigma rule)
- [ ] **High network I/O**: inbound/outbound traffic >5x normal baseline
- [ ] **Service latency**: response time >5x normal for `svc-automation` or `svc-gateway` (Datadog APM)
- [ ] **Connection timeout**: Datadog loses contact with `svc-monitor` agent

### 4.2 Edge Security Provider Alerts

- [ ] **DDoS attack detected**: Cloudflare dashboard shows active mitigation; Sigma rule `cloudflare/waf-block-burst.yml`
...
- [ ] **User reports**: services at `n8n.example-ops.com` unreachable or slow
...
- [ ] **Step 2.2**: If the attack is bypassing automatic mitigation, enable "Under Attack" mode:
 ...
 ```
 As of 2026, "Under Attack" applies Cloudflare's Managed Challenge (Turnstile) rather than the legacy JavaScript challenge, which stops most L7 attacks while letting legitimate browsers through.

- [ ] **Step 2.3**: Implement rate limiting on Cloudflare. Path-scope the rule to the endpoint under attack rather than the whole hostname; webhook endpoints can take heavy bursts during normal automation runs.
 ```
 Cloudflare dashboard > Security > WAF > Rate Limiting Rules
 - Add rule (path-scoped): If `http.request.uri.path eq "/webhook/master-cmd"` and requests from single IP > 100/minute to `n8n.example-ops.com`, block for 10 minutes
 ```
...
- [ ] **Step 2.5**: If the tunnel is overwhelmed, restrict tunnel ingress. Confirm the host has the `xt_connlimit` kernel module before relying on `connlimit`.
 ```bash
 # On alpha-node, verify connlimit availability and then add iptables rules
 ssh root@10.100.1.10 << 'EOF'
 lsmod | grep -q connlimit || modprobe xt_connlimit || echo "(connlimit unavailable; use --limit only)"
...
- [ ] **Step 2.6**: If the VPS itself is being directly targeted (bypassing edge security). Note: the instance has a public IP (`10.100.1.10` sanitized), so direct origin reach is technically possible.
 ```bash
 ...
 # This is a verification step. Current state has every service port bound to
 # 127.0.0.1, so any 0.0.0.0 binding seen here is a deviation to fix.
 ```
...
- [ ] **Step 3B.4**: If memory is critically low, free memory by stopping non-critical services:
 ...
 # svc-ai-gateway runs standalone
 docker stop svc-ai-gateway
 ```

- [ ] **Step 3B.5**: Clear Docker build cache and unused images. Omit the `--volumes` flag entirely; passing `--volumes` deletes volumes, omitting it preserves them. There is no `--volumes=false` form.
 ```bash
 docker system prune -f
 # WARNING: Do NOT pass --volumes; that deletes persistent data.
 ```
...
- [ ] **Step 3C.3**: If PostgreSQL WAL/data is consuming disk:
 ```bash
 docker exec svc-db psql -U "$CD_DB_USER" -c "SELECT pg_size_pretty(pg_database_size('<db_name>'));"
 # VACUUM FULL takes an EXCLUSIVE lock on the table and rewrites it; running
 # it during degradation can make things worse. Only run during off-peak windows
 # or when the alternative is total disk exhaustion. Prefer plain VACUUM first.
 docker exec svc-db psql -U "$CD_DB_USER" -c "VACUUM (VERBOSE, ANALYZE);"
 # docker exec svc-db psql -U "$CD_DB_USER" -c "VACUUM FULL;"   # off-peak only
 ```
...
- [ ] **Step 4.2**: Check service endpoints are responding:
 ```bash
 # Automation platform (n8n; port 5678 in this stack)
 ...
 # Database
 docker exec svc-db pg_isready -U "$CD_DB_USER"
 ```

- [ ] **Step 4.3**: Verify the tunnel is operational:
 ```bash
 # From external: test that n8n.example-ops.com resolves and responds
...
- [ ] **Step 4.6**: If non-critical services were stopped during containment, restart them:
 ```bash
 docker compose up -d svc-llm svc-transcription
 # svc-ai-gateway is standalone:
...
- [ ] **Step 5.6**: Capacity planning. The Phase 17 stack is RAM-heavy and runs close to the 8 GB ceiling. Approximate steady-state RAM accounting:
 - Phase 17: NeMo 1G + Langfuse-web 1G + Langfuse-worker 0.5G + ClickHouse 1G + Redis 0.25G + Squire 0.3G = ~4G
 - Core: n8n 2G + db 1G + vault 0.5G + keycloak 1G + teleport 0.5G + datadog 0.5G + falco 0.5G = ~6G
 - Total floor is roughly the size of the 8GB ceiling, leaving very little headroom. Document the floor and surface this as a known fragility.
...
| 15 (Medium) | `svc-fluentd` | Log aggregation for Phase 17 stack; can buffer |
| 16 (Medium) | `svc-teleport-event-handler` | Teleport audit shipper; can buffer briefly |
| 17 (Medium) | `svc-langfuse-clickhouse` | Trace store for Squire; pause writes degrades but does not block recommend path |
| 18 (Medium) | `svc-langfuse-redis` | Queue cache for Langfuse worker |
| 19 (Medium) | `svc-langfuse-worker` | Trace ingestion worker; backpressure tolerated |
| 20 (Medium) | `svc-langfuse-web` | Langfuse UI (observability); user-facing |
| 21 (Medium) | `svc-nemo` | NeMo Guardrails rail stack for Squire |
| 22 (High) | `svc-squire` | Phase 17 recommend-only SOC analyst; degrades AI capability but not core platform |

> `svc-squire` and `langfuse.example-ops.com` both have public Cloudflare ingress in the current state. Update Section 2 (Scope) when this changes.
...
2. DDoS:    Edge security handles L3/L4 automatically
        ...
        - Add path-scoped rate limiting rules
        - Do NOT expose origin IP

3. DEGRADE: Identify bottleneck (CPU / mem / disk)
        - Stop non-critical services: svc-llm, svc-transcription (compose); svc-ai-gateway (docker stop)
        - Restart offending container
        - Clean disk: docker image prune -f, then truncate logs
