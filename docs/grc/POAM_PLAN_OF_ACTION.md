...
| POAM-P17-15 | Red-team cycle 2 returned HTTP 500 on 3 of 14 cases (08, 16, 18) when concurrency 5 hit the Anthropic 30k input-tokens-per-minute + 8k output-tokens-per-minute cap on Opus 4.7. The Ollama fallback in llm_backend cannot reach `cd-service-ollama` because that service lives on the `net-ai` internal docker network with no reachable route from `net-core`. Failure was availability-class, not integrity; no severity drift occurred because the graph exited before rendering. Remediation: (a) cap red-team runner default concurrency to 2 and add a per-case token pre-check; (b) wire the llm_backend Ollama path to use the docker bridge IP or dual-attach `cd-service-ollama` to `net-core`; (c) evaluate Anthropic rate-tier lift if bursty load pattern persists. Acceptance rationale: demo-scope availability is non-critical, and the failure mode is loud (5xx) rather than silent severity drift. | MED | System Owner | 2026-Q3 | OPEN | `REDTEAM_RESULTS.md` Finding 7 (cycle 2 cases 08, 16, 18) |
