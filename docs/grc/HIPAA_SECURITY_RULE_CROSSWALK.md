...
| Encryption and Decryption | Addressable | 164.312(a)(2)(iv) | All data at rest on `alpha-node` is encrypted via DigitalOcean volume encryption (AES-256, provider-managed keys). Database volumes are also encrypted at the filesystem layer where supported. **Gap:** Dedicated ePHI customer-managed key (CMK) workflow not yet implemented; will be added if a healthcare customer requires CMK (see Section 12, Gap G-01). | SC-13, SC-28, SC-28(1) |
...
| Implementation Specification | Status | 45 CFR | OSOP Implementation | NIST 800-53 Controls |
|------------------------------|--------|--------|---------------------|----------------------|
| Audit Controls | Required | 164.312(b) | Multi-layer audit architecture per [SSP_SYSTEM_SECURITY_PLAN.md](SSP_SYSTEM_SECURITY_PLAN.md) Section 6 and [AI_AUDIT_TRAIL_SPEC.md](AI_AUDIT_TRAIL_SPEC.md). Falco for syscall and runtime events. `svc-gateway` for session recordings. Application logs from all services routed through the log shipper to Datadog. Database query logs for `svc-db`. AI-specific Langfuse traces per [GUARDRAILS_CONFIGURATION.md](GUARDRAILS_CONFIGURATION.md). **Gap:** Audit log retention currently 90 days online plus 1 year archive; HIPAA requires retention for 6 years per 164.316(b)(2)(i). Extended retention plan in Section 12, Gap G-03. | AU-2, AU-3, AU-3(1), AU-6, AU-9, AU-11, AU-12 |
...
| Implementation Specification | Status | 45 CFR | OSOP Implementation | NIST 800-53 Controls |
|------------------------------|--------|--------|---------------------|----------------------|
| Person or Entity Authentication | Required | 164.312(d) | Human authentication: `svc-gateway` (Teleport) with FIDO2 hardware key or TOTP plus password. Service authentication: mutual TLS where supported, service tokens elsewhere, all secrets managed in Doppler with quarterly rotation. API authentication: bearer tokens, HMAC-signed webhook payloads, and IP allow-listing per route. | IA-2, IA-2(1), IA-2(2), IA-2(8), IA-3, IA-5 |
...
| Subcontractors | Required | 164.314(a)(2)(iii) | OSOP's downstream service providers that would handle ePHI (currently DigitalOcean for infrastructure, Anthropic for AI inference if AI workloads touch ePHI) would be evaluated for HIPAA capability. DigitalOcean offers a BAA for eligible customers. Anthropic's enterprise contract supports BAAs. At first BAA signing, downstream BAAs will be executed before ePHI flows. | SA-9, SA-9(2), SR-3, SR-3(1) |
